...
size_t modelFileId(Race race, LookSlot slot, uint16_t modelId)
{
    ...
    // An empty hand. Zero is a real model everywhere else - face 0 is a face
    // - but no weapon in the item table has model zero, so for a weapon it can
    // only mean nothing is held. Without this the file at the foot of the
    // weapon window is a real mesh, and every unarmed character would carry it.
